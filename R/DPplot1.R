...
DPplot1=function(object, s = s, id_new=id_new, n.chains = n.chains, n.iter = n.iter, n.burnin = floor(n.iter / 2),
                 dataLong, dataSurv){
  ...
  Dt=seq(s,max(time_new),.3)

  est_M=matrix(0,length(Dt),4)

for(kk in 1:length(Dt)){
  DD <- DP_CV_CI(
    object = Z2, s = s, t = Dt[kk], mi=2, n.chains = 1, n.iter = n.iter, n.burnin = n.burnin,
  ...
  plot(TT[,1], TT[,5], type="p", pch=20, xaxt="n",col="black",ylab="Marker",xlab="Time",
       main="")
  ...
  lines(TT[,c(1,3)], type="l",col="purple3", lty=2,pch=16, axes=F, xlab=NA, ylab=NA, cex=1)
  lines(TT[,c(1,4)], type="l", col="purple3", lty=2, pch=16, axes=F, xlab=NA, ylab=NA, cex=1)
